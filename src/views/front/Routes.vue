      ...
      <el-row :gutter="20">
        <el-col :span="8" v-for="item in data.routesList" :key="item.id" style="margin-bottom: 20px">
          <a :href="'/front/routesDetail?id=' + item.id">
            <div>
              <!-- ✅ 修复：图片加上跨域属性，解决 403 防盗链 -->
              <img :src="item.img" alt="" style="width: 100%; height: 400px" crossorigin="anonymous">
              <div style="height: 50px; text-align: justify" class="line2 title">{{ item.name }}</div>
            </div>
          </a>
        </el-col>

        <div v-if="!data.routesList.length" style="text-align: center; color: #666; padding: 40px">
          暂无旅游路线数据
        </div>
      </el-row>
